...
configCommand=params.get("configCommand");
configTemplate=params.get("configTemplate");

if (configCommand!=null) {

    if (params.get("mode")!=modes["logedInConfigMode"]) {

        configModeResult = setConfigMode();

        if (configModeResult.get("status") == status["success"]) {

            result = sendConfigCommand(configCommand);
        } else {
    ...
    if (params.get("mode")!=modes["logedInConfigMode"]){
        ...
        if (configModeResult.get("status") == status["success"]) {
            ...
            result = applyConfigTemplate(configTemplate);
        } else {
    ...
    return ["status": status["failure"], "data": "No config command or config template passed","mode": mode]
